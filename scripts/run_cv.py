...
def main():
    ...
    for seed in args.seeds:
        ...
        for k, (tr, va) in enumerate(kf.split(all_idx, labels)):
            ...
            # 该目录下子目录 = run_id_fold_x_train, 找最新
            sub_runs = sorted([d for d in inner_dir.iterdir() if d.is_dir()],
                                key=lambda d: d.stat().st_mtime, reverse=True)
            if not sub_runs:
                print(f"[!!] no training_log found in {inner_dir}")
                fold_results.append({
                    "seed": seed, "fold": k, "status": "no_log",
                })
                continue
            log_dir = sub_runs[0] / args.model
            log_path = log_dir / "training_log.json"
            test_path = log_dir / "test_metrics.json"
            entry = {"seed": seed, "fold": k, "log_dir": str(log_dir),
                     "status": "ok"}
            if log_path.exists():
                ...
                entry["total_training_seconds"] = log["total_training_seconds"]
            if test_path.exists():
                with open(test_path) as f:
                    tm = json.load(f)
                entry["test_accuracy"] = tm["test_accuracy"]
                entry["macro_f1"] = tm["macro_f1"]
                entry["weighted_f1"] = tm["weighted_f1"]
    ...
    val_accs = [f["best_val_accuracy"] for f in fold_results
                  if "best_val_accuracy" in f]
    macro_f1s = [f["macro_f1"] for f in fold_results if "macro_f1" in f]
    if test_accs:

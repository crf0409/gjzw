...
from keras.applications import InceptionV3  # 改为InceptionV3
import numpy as np
import pandas as pd
import os
import matplotlib
...
class AncientImageClassifier:
    ...
    def load_data(self):
        ...
        if self.img_height is None or self.img_width is None:
            ...
            print(f"注意: InceptionV3推荐输入尺寸为299×299，当前使用 {self.img_width}×{self.img_height}\n")
    ...
    def build_model(self):
        ...
        print(f"✓ InceptionV3预训练权重加载成功 (ImageNet)")
        
        # 微调策略：InceptionV3可以分阶段解冻
        ...
        print(f"InceptionV3: 总层数 {len(base_model.layers)}, 冻结前 {fine_tune_at} 层, 可训练层数 {trainable_count}")
        ...
        model = keras.Model(inputs=inputs, outputs=outputs, name='InceptionV3_Classifier')
        
        # InceptionV3优化器配置
        # 使用RMSprop（InceptionV3原始论文推荐）
        ...
        self.model = model
        print("\n=== InceptionV3 微调模型 ===")
        print(f"基础模型: InceptionV3 (ImageNet预训练)")
        print(f"架构特点: 多尺度特征提取 (Inception模块)")
        print(f"模型大小: ~92MB")
        print(f"ImageNet Top-1准确率: ~77.9%")
        print(f"ImageNet Top-5准确率: ~93.7%")
        print(f"推荐输入尺寸: 299×299")
    ...
    def train(self, X_train, X_val, y_train, y_val, epochs=100):
        ...
        print("\n" + "="*60)
        print("InceptionV3 微调训练")
        print("="*60)
        print("训练参数：")
        print("  - Optimizer: RMSprop (原始论文推荐)")
        print("  - Momentum: 0.9")
        print("  - RMSprop Decay: 0.9")
        print("  - Initial Learning Rate: 0.0001")
        print("  - Learning Rate Decay: Exponential + ReduceLROnPlateau")
        print("  - Weight Decay: 0.0001 (L2)")
        print(f"  - Batch Size: {self.batch_size}")
        print("  - Dropout: 0.2-0.4 (多层)")
        print("="*60 + "\n")
    ...
    def plot_training_history(self):
        ...
        axes[0].set_title('Model Accuracy (InceptionV3)', fontsize=14)
        ...
        axes[1].set_title('Model Loss (InceptionV3)', fontsize=14)
        ...
        plt.tight_layout()
        plt.savefig('training_history_inceptionv3.png', dpi=300, bbox_inches='tight')
        print("\n训练历史图已保存: training_history_inceptionv3.png")
    ...
    def evaluate_test_set(self):
        ...
        print(f"  InceptionV3 ImageNet Top-1: 77.9%")
        print(f"  InceptionV3 ImageNet Top-5: 93.7%")
        ...
        print("混淆矩阵图已保存: confusion_matrix_inceptionv3.png")
    ...
    def predict_sample_images(self, num_samples=9):
        ...
        plt.tight_layout()
        plt.savefig('predictions_inceptionv3.png', dpi=300, bbox_inches='tight')
        print("\n预测结果已保存: predictions_inceptionv3.png")
...
if __name__ == "__main__":
    ...
    print("="*60)
    print("古代文字图像分类训练")
    print("架构: InceptionV3 (Fine-tuning)")
    print("参数量: ~23.8 Million")
    print("ImageNet Top-1准确率: 77.9%")
    print("核心创新: 多尺度特征提取 (Inception模块)")
    print("推荐输入尺寸: 299×299")
    print("="*60)
    ...
    classifier.model.save('final_model_inceptionv3.keras')
    print("\n最终模型已保存: final_model_inceptionv3.keras")
    print("="*60)
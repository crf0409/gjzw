...
from keras.applications import EfficientNetB3  # 改为EfficientNetB3
import numpy as np
import pandas as pd
import os
import matplotlib
...
class AncientImageClassifier:
    ...
    def load_data(self):
        ...
        if self.img_height is None or self.img_width is None:
            ...
            print(f"注意: EfficientNetB3推荐输入尺寸为300×300，当前使用 {self.img_width}×{self.img_height}\n")
    ...
    def build_model(self):
        ...
        print(f"✓ EfficientNetB3预训练权重加载成功 (ImageNet)")
        
        # 微调策略：EfficientNet参数高效，可以微调较多层
        ...
        print(f"EfficientNetB3: 总层数 {total_layers}, 冻结前 {fine_tune_at} 层, 可训练层数 {trainable_count}")

        # 添加分类头
        x = base_model.output
        x = layers.BatchNormalization(name='bn_fc')(x)
        x = layers.Dropout(0.3, name='dropout1')(x)
        x = layers.Dense(512, activation='swish',  # EfficientNet使用swish激活
        ...
        print("\n=== EfficientNetB3 微调模型 ===")
        print(f"基础模型: EfficientNetB3 (ImageNet预训练)")
        print(f"架构特点: 复合缩放 (Compound Scaling) + MBConv + SE注意力")
        print(f"模型大小: ~48MB")
        print(f"ImageNet Top-1准确率: ~81.6%")
        print(f"ImageNet Top-5准确率: ~95.7%")
        print(f"推荐输入尺寸: 300×300")
        ...
        print("\nEfficientNetB3优势:")
        print("  ✓ 参数效率极高 (仅12M参数)")
        print("  ✓ 准确率高 (81.6% Top-1)")
        print("  ✓ 复合缩放优化深度、宽度、分辨率")
        print("  ✓ 使用神经架构搜索(NAS)")
        print("  ✓ SE注意力机制")
        print("  ✓ 平衡性能和效率的最佳选择")
        model.summary()
    ...
    def train(self, X_train, X_val, y_train, y_val, epochs=100):
        ...
        print("\n" + "="*60)
        print("EfficientNetB3 微调训练")
        print("="*60)
        print("训练参数：")
        print("  - Optimizer: Adam")
        print("  - Initial Learning Rate: 0.0001")
        print("  - Learning Rate Decay: Cosine Annealing")
        print("  - Weight Decay: 0.0001 (L2)")
        print(f"  - Batch Size: {self.batch_size}")
        print("  - Dropout: 0.2-0.3")
        print("  - Activation: Swish (EfficientNet标配)")
        print("="*60 + "\n")
        ...
        callbacks = [
            ...
            keras.callbacks.ModelCheckpoint(
                'best_model_efficientnetb3.keras',
    ...
    def plot_training_history(self):
        ...
        axes[0].set_title('Model Accuracy (EfficientNetB3)', fontsize=14)
        ...
        axes[1].set_title('Model Loss (EfficientNetB3)', fontsize=14)
        ...
        plt.tight_layout()
        plt.savefig('training_history_efficientnetb3.png', dpi=300, bbox_inches='tight')
        print("\n训练历史图已保存: training_history_efficientnetb3.png")
    ...
    def evaluate_test_set(self):
        ...
        print(f"  EfficientNetB3 ImageNet Top-1: 81.6%")
        print(f"  EfficientNetB3 ImageNet Top-5: 95.7%")
        ...
        plt.savefig('confusion_matrix_efficientnetb3.png', dpi=300, bbox_inches='tight')
        print("混淆矩阵图已保存: confusion_matrix_efficientnetb3.png")
    ...
    def predict_sample_images(self, num_samples=9):
        ...
        plt.tight_layout()
        plt.savefig('predictions_efficientnetb3.png', dpi=300, bbox_inches='tight')
        print("\n预测结果已保存: predictions_efficientnetb3.png")
...
if __name__ == "__main__":
    ...
    print("="*60)
    print("古代文字图像分类训练")
    print("架构: EfficientNetB3 (Fine-tuning)")
    print("参数量: ~12 Million")
    print("模型大小: ~48MB")
    print("ImageNet Top-1准确率: 81.6%")
    print("核心技术: 复合缩放 (Compound Scaling) + NAS + SE")
    print("推荐输入尺寸: 300×300")
    print("="*60)
    print("EfficientNet系列特点:")
    print("  - 参数效率最高的模型系列")
    print("  - 通过复合缩放同时优化深度、宽度、分辨率")
    print("  - 在准确率和效率之间取得最佳平衡")
    print("="*60)
    ...
    classifier.model.save('final_model_efficientnetb3.keras')
    print("\n最终模型已保存: final_model_efficientnetb3.keras")
    print("="*60)